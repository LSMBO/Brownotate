...
from flask_app.process_manager import check_process, get_cpus_used
...
def get_user_annotations():
    ...
    for annotation in annotations['data']:
        ...
        if check_processes and annotation['status'] == 'running' and 'type' in annotation['parameters'] and annotation['parameters']['type'] == 'functional':
            process_found = check_process(run_id)
            if not process_found:
                annotation['status'] = 'failed'            
        elif check_processes and annotation['status'] == 'running' and 'progress' in annotation and annotation['progress'][-1] not in progress_not_using_process and not user.startswith('workshop-cjfps'):
            process_found = check_process(run_id)
            if not process_found:
                annotation['status'] = 'failed'
